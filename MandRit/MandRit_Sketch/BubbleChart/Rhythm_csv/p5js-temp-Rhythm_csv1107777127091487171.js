...
function preload(){
  airData = loadTable("csv_musics/Plotar_Musicas/TocandoemFrente_3por4TIPO1.csv",
    "csv",
    "header");
    
     sound = loadSound('ArquivoMusicas/Dance_Monkey2.mp3');
...
function drawCirclefundo(){
  ...
  //Opção Nogrid:
    ...
    circles = 16; //8 - 2por2 //16 - 2por4 //32 - 3por4 //64 - 4por4 maioria das músicas //TakeaFive 5/4 = 128 tempos
...
function animarcursor() {
  ...
  beginShape();
  for (var p = 0; p <  360; p++) {
    var r =  map(ocorrencia[p], height/2, width/2, 10,400);
    var x = r * cos(p);
    var y = r * sin(p);
    ellipseMode(CENTER);
       stroke(146);
